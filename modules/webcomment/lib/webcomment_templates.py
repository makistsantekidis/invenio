...
""" % {'form': write_button_form,
    ...
    def tmpl_get_comment_without_ranking(self, req, ln, nickname, comment_uid,
                                         date_creation, body, body_format, status,
                                         nb_reports, reply_link=None,
                                         report_link=None, undelete_link=None,
                                         delete_links=None, unreport_link=None,
                                         recID=-1, com_id='',
                                         attached_files=None, collapsed_p=False,
...
<div class="webcomment_comment_box">
    ...
    def tmpl_get_comment_with_ranking(self, req, ln, nickname, comment_uid,
                                      date_creation, body,  body_format, status,
                                      nb_reports, nb_votes_total,
                                      nb_votes_yes, star_score, title,
                                      report_link=None, delete_links=None,
                                      undelete_link=None, unreport_link=None,
                                      recID=-1, associated_files = None):
        """
        private function
        @param req: request object to fetch user info
        @param ln: language
...
%(abuse)s''' % {'baseurl'      : CFG_BASE_URL,
    ...
    <div>"""
                ...
                if not reviews:
                    ...
                    comments_rows += self.tmpl_get_comment_without_ranking(req, ln, messaging_link, comment[c_user_id],
                                                                           comment[c_date_creation], comment[c_body], comment[c_body_format],
                                                                           comment[c_status], comment[c_nb_reports],
                                                                           reply_link, report_link, undelete_link,
                                                                           delete_links, unreport_link,
                                                                           recID, comment[c_id], files,
                    ...
                    undelete_link = "%s/admin/webcomment/webcommentadmin.py/undel_com?ln=%s&amp;id=%s" % (CFG_SITE_URL, ln, comment[c_id])
                    unreport_link = "%s/admin/webcomment/webcommentadmin.py/unreport_com?ln=%s&amp;id=%s" % (CFG_SITE_URL, ln, comment[c_id])
                    comments_rows += self.tmpl_get_comment_with_ranking(req, ln, messaging_link, comment[c_user_id],
                                                                        comment[c_date_creation], comment[c_body], comment[c_body_format],
                                                                        comment[c_status], comment[c_nb_reports],
                                                                        comment[c_nb_votes_total],
                                                                        comment[c_nb_votes_yes],
                                                                        comment[c_star_score], comment[c_title],
                                                                        report_link, delete_links, undelete_link,
                                                                        unreport_link, recID,
                                                                        associated_files=associated_files)
...
''' % {'url': url,
    ...
    def tmpl_comment_filtering_box_and_script(self, more_exist_p, filter_query='', total_nb=100, recID=None ,ln=CFG_SITE_LANG):
        ...
        filter_comments_url = '/record/%(recID)s/comments/display?ln=%(ln)s&filter=' \
                % {
                    'recID'        : recID,
                    'ln'           : ln
                  }
        ...
        var $select = $('#selectize_input').selectize({
        ...
        var filter_all_url = "%(filter_comments_url)s";
        var current_page = parseInt(getParameterByName('p'));
        ...
        function filter() {
          ...
          file_query = "@file_filter:" + $("#selectize_input").val()
          $("#all_comments_anchor").attr("href",filter_all_url+query+file_query)
          ...
          if (%(has_more_pages)s)
          {
            ...
            $("#cmtRound").append("<div id='search_next_page' style='text-align:center'><a id='search_all_authors_anchor' align='center' style='font-size:18px;margin-left:auto;margin-right:auto;' href='"+filter_all_url+query+file_query+"&p="+ p +"'>Click here to search the comments of the next page<a></div>")
          }
          ...
          $(function() {
             if (%(has_more_pages)s)
             {
                $("#filter_area").append("<br /><a id='all_comments_anchor' style='font-size:13px' href='%(filter_comments_url)s%(filter_query)s&nb=%(nb)s'>Use this filter for all record's comments</a>")
             }

             if ("%(filter_query)s")
             {
                query = "%(filter_query)s".split("@file_filter:")
                $("#filter_input").val(query[0])
                if (query.length > 1)
                {
       ...
       """ % { 'has_more_pages'          : more_exist_p and "true" or "false",
               ...
               'filter_comments_url' : filter_comments_url,
