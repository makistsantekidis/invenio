...
def check_record(record, overwrite=True):
    ...
    if not overwrite and get_wiki_link_from_bfo(record):
        ...
        for control_no in control_nos:
            if (control_no.find("|(VIAF)") != -1):
                ...
                if link:
                    linkfield = ([(CFG_VIAF_WIKIPEDIA_NAME_LABEL_SUBFIELD,CFG_VIAF_WIKIPEDIA_NAME_VALUE_SUBFIELD),(CFG_VIAF_WIKIPEDIA_LINK_SUBFIELD,link)] , '', '', ' ', maxi)
                    if get_wiki_link_from_bfo(record):
                        for field in record[CFG_VIAF_WIKIPEDIA_LINK_BFO_FIELD]:
                            for subfield in field:
                                if type(subfield) is list and subfield[0] == CFG_VIAF_WIKIPEDIA_NAME_LABEL_SUBFIELD and subfield[1] == CFG_VIAF_WIKIPEDIA_NAME_VALUE_SUBFIELD:
                                    for sub in field:
                        ...
                        record_add_field(record,CFG_VIAF_WIKIPEDIA_LINK_BFO_FIELD, \
                                subfields=[(CFG_VIAF_WIKIPEDIA_NAME_LABEL_SUBFIELD,CFG_VIAF_WIKIPEDIA_NAME_VALUE_SUBFIELD),(CFG_VIAF_WIKIPEDIA_LINK_SUBFIELD,link)])
